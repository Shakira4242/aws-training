...
# Our training data follows the "interpolated.b" format from Ross Wightman's scripts.
...
(train_seq, valid_seq), (mean, std) = process_csv(filename="interpolated1.csv", val=5) # concatenated interpolated.csv from rosbags 
# print(process_csv(filename="data/interpolated.csv"))
# test_seq = read_csv("../../../Downloads/output/interpolated.csv") # interpolated.csv for testset filled with dummy values
...
with graph.as_default():
    ...
    input_images = tf.pack([tf.image.decode_jpeg(tf.read_file(x)) for x in tf.unpack(tf.reshape(inputs, shape=[(LEFT_CONTEXT+SEQ_LEN) * BATCH_SIZE]))])
